...
soil.entry<-function()
{
	...
	setwd('C:/users/vijay/documents/my dropbox/alaskafiles/2011 datasheets protocols')
	...
	while(choice=='y'|choice=='Y')
	{
		...
		stage<-readLines(n=1)
			if(stage=='1')
			{
				...
				cat("Weigh boat tare: ")
				WeighboatTareFroze<-readLines(n=1)
				...
				froze.df<-data.frame(TransectID,Stratum,LakeID,FocalNum,CommunityType,Layer,FrozenTotalWeight,WeighboatTareFroze,AirDryStart,NotesFroze,stringsAsFactors=FALSE)
			...
			if(stage!='1')
			{
				...
				q=paste("Select ID,",names," from SoilMoisture where ",names," in (",vals,")",sep='')
				a<-sqlQuery(yrb2011,q)
				print(a)
			
				cat("\nEnter the line to modify")
				...
				{
					cat("\nWeigh Boat Tare")
					WeighBoatTareAirDried=as.numeric(readLines(n=1))
					...
					airdry.df<-data.frame(ID=line,WeighBoatTareAirDried,AirDryAll,AirDrySub,OvenTemp,NotesAirDry,OvenDryStart)
