...
veg.spp.entry<-function()
{
	...
	while(choice=='y'|choice=='Y')
	{
		...
		transect<-sqlFetch(yrb,'tblTransects')
...
#allspp is df version of tblVegSpeciesList
	...
	#could add in to add species name, but probably not worth it.
	
	name.df<-data.frame(Species=ID,stringsAsFactors=FALSE)
	return(name.df)
}
...
HabitatTypeInput2<-function()
{
	...
	ht.df<-data.frame(rbind(as.numeric(h.bool)),row.names=FALSE,stringsAsFactors=FALSE)
	names(ht.df)<-ht.n
...
TransectIDinput<-function(transects.df,entry.df)
{
		...
		Transect.df<-data.frame(TransectID,stringsAsFactors=FALSE)
		return(Transect.df)
}
